...
def compute_quality_flags(
    ...
    Параметры:
    - high_cardinality_threshold: порог уникальных значений для категориальных признаков
    - zero_share_threshold: порог доли нулей в числовых колонках
    """
    flags: Dict[str, Any] = {}
    ...
    # 3. Проверка на дубликаты в id-колонках (колонки с "id" в названии)
    suspicious_id_columns: List[str] = []
    if df is not None:
        for col in summary.columns:
            col_name_lower = col.name.lower()
            if "id" in col_name_lower or col_name_lower.endswith("_id"):
                # Проверяем, есть ли дубликаты
                if df[col.name].dropna().duplicated().any():
    ...
    # 4. Проверка на большую долю нулей в числовых колонках
    ...
    flags["high_zero_columns"] = high_zero_columns
    ...
    if flags["has_suspicious_id_duplicates"]:
        score -= 0.1 * len(suspicious_id_columns)
    if flags["has_many_zero_values"]:
        score -= 0.03 * len(high_zero_columns)
